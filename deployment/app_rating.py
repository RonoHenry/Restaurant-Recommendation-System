...
def ratings():
    with st.container(border=True):
        ...
        if not st.session_state.ratings_confirmed:
            collect_ratings(all_restaurants_df[all_restaurants_df.state == state])

            if st.button("Confirm Ratings"):
                with st.spinner('Cooking something up for you...'):
                    st.session_state.ratings_confirmed = True
                    time.sleep(2)
                    st.session_state.recommendations = recommend_restaurants(user_id, st.session_state.ratings, all_restaurants_df, state)
